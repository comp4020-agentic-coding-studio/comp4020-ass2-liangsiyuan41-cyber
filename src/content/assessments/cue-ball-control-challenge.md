---
title: "Assessment 2: Cue Ball Control Challenge"
description: >
  A hands-on test of stop, follow, draw and side spin — judged on where the
  cue ball finishes, not just on whether the object ball is potted.
week: 8
due: 2027-04-14T12:00:00+10:00
weight: 25
marking:
  mode: weighted
  criteria:
    - name: Cue-ball finishing position
      weight: 70
    - name: Consistency across attempts
      weight: 30
spec:
  - you can play stop, follow and draw shots on request
  - you can land the cue ball within a marked zone at least half the time
related:
  - three-shots-ahead-challenge
---

## The brief

> Play a short set of shots called by the assessor, each specifying not just
> the object ball but where the cue ball should finish.

Potting is assumed at this stage — the mark is for control of the cue ball
afterwards.

## What you submit

Demonstrated live at the table during a practical session; no written
submission.
